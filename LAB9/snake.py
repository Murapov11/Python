...
class Food:
  ...
  def generate_random_pos(self):
    self.x_ = random.randint(20,40)
    self.y_ = random.randint(20,40)
  ...
  # the drawing a food 
  def draw(self):
    pygame.draw.rect(screen, BLUE, (self.x, self.y, self.x_, self.y_))

  def weight(self):
      if self.x_ + self.y_ > 60:
          return 3
      if self.x_ + self.y_ > 40:
          return 2
      else:
          return 1
...
# Creating a USER event to set timer 
...
while running:
  for event in pygame.event.get():
    if event.type == NEW_FOOD:
        ...
        pygame.time.set_timer(NEW_FOOD, 10000) # set timer
