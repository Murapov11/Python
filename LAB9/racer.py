...
class Coin(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        # randomly generate different weigt coins...
        self.width = random.randint(25,75)
        self.height = random.randint(25,75)

        self.image = pygame.Surface((self.width,self.height)) # create a surface
    ...
    # POINTS TO THE earn coin
    def weight(self):
        if self.width + self.height > 100:
            return 3
        elif self.width + self.height > 50 :
            return 2
        else:
            return 1
...
while True:
    #condition to increase speed of enemy
    if cnt >= 5 * x:
        SPEED += 0.5
        x += 1
    ...
    earnpoints = font_small.render(f'POINTS :{cnt}',True,BLACK)
    # bliting the surfaces
    DISPLAYSURF.blit(scores, (10,10))
    DISPLAYSURF.blit(earnpoints,(280,10))
    ...
    if pygame.sprite.spritecollide(P1,points,True):
        cnt += C1.weight() # increase the count 
        print(cnt)
